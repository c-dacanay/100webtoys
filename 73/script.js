...
let colors = ['#292B2C', '#E298E4', '#2e94b0', '#7B67F3', '#B6FF8E', '#FBFF7B','#FA5F66']
let c = 0;

function setup() {
  createCanvas(windowWidth, windowHeight);
  slider = createSlider(-10, 10, 0);
  slider.position(width/2 - 100, 5)
 
    button = createButton('change color')
    button.position(width/2 - 80, 45)
...
function draw() {
  ...
  for (let i = 0; i < sines.length; i++) {
    let erad = 1; // radius for small "point" within circle... this is the 'pen' when tracing
    // setup for tracing
    if (trace) {
      stroke(255, alpha); // blue
      fill(col, alpha / 2);
